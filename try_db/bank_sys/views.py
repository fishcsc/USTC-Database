...
def clients(request):
    ...
    if request.method == "GET":
        ...
        return render(request, 'bank_sys/clients.html', context)
...
#         client_id=request.POST.get('client_id')
#         if not client_id:
#             return render(request, 'bank_sys/add_client.html', {'error': '输入不能为空'})
#         if Client.objects.filter(id = client_id):
#             return render(request, 'bank_sys/add_client.html', {'error': '该用户已存在'})

#         client_name=request.POST.get('client_name')
#         client_phone=request.POST.get('client_phone')
#         if Client.objects.filter(phone = client_phone):
#             return render(request, 'bank_sys/add_client.html', {'error_ph': '该号码已存在'})
...
#     return render(request, 'bank_sys/add_client.html')
...
def add_client(request):
    if request.method == "POST":
        ...
        # 检查员工是否存在
        try:
            staff = Staff.objects.get(id=client_staff_id)
        except Staff.DoesNotExist:
            return render(request, 'bank_sys/add_client.html', {'error': '指定的员工不存在'})
...
def edit_client(request, client_id):
    ...
    if request.method == "POST":
        ...
        if (new_client_id != client_id) and Contact.objects.filter(client_id = client_id):
            return render(request, 'bank_sys/edit_client.html', {'obj' : obj, 'error': '该用户有关联信息，不能修改身份证号'})
        if (new_client_id != client_id) and Client.objects.filter(id=new_client_id):
            return render(request, 'bank_sys/edit_client.html', {'obj' : obj, 'error': '该身份证号已存在'})
        ...
        return redirect('../../clients')
    return render(request, 'bank_sys/edit_client.html', {'obj' : obj})
...
def contacts(request, client_id):
    if request.method == "GET":
        ...
        return render(request, 'bank_sys/contacts.html', context)
    # 查询
    if request.method == "POST":
...
def add_contact(request, client_id):
    ...
    if request.method == "POST":
        contact_name = request.POST.get('contact_name')
        if not contact_name:
            return render(request, 'bank_sys/add_contact.html', {'error': '输入不能为空', 'client_id': client_id})
        if Contact.objects.filter(**{'name':contact_name, 'client_id':client_id}):
            return render(request, 'bank_sys/add_contact.html', {'error': '该联系人已存在', 'client_id': client_id})
        contact_phone = request.POST.get('contact_phone')
        if Contact.objects.filter(**{'phone':contact_phone}):
            return render(request, 'bank_sys/add_contact.html', {'error_ph': '该号码已存在', 'client_id': client_id})
        contact_email = request.POST.get('contact_email')
        if Contact.objects.filter(**{'email':contact_email}):
            return render(request, 'bank_sys/add_contact.html', {'error_em': '该email已存在', 'client_id': client_id})
    ...
    return render(request, 'bank_sys/add_contact.html', {'client_id': client_id})
...
def edit_contact(request, client_id, contact_name):
    ...
    if request.method == "POST":
        ...
        if not contact_name_:
            return render(request, 'bank_sys/edit_contact.html', {'error': '输入不能为空', 'client_id': client_id, 'contact_name':contact_name})
        contact_phone=request.POST.get('contact_phone')
        contact_email=request.POST.get('contact_email')
        contact_relation = request.POST.get('contact_relation')
        ...
        url = '/clients/contacts/'+client_id
        return HttpResponseRedirect(url)
    return render(request, 'bank_sys/edit_contact.html', {'obj':obj, 'client_id': client_id, 'contact_name':contact_name})
...
def accounts(request):
    ...
    if request.method == "GET":
        ...
        return render(request, 'bank_sys/accounts.html', {'accounts':accounts, 'accessaccounts':accessaccounts})
    # 查询
...
def add_checking(request):
    if request.method == "POST":
        ...
        if not subbranch_name:
            return render(request, 'bank_sys/add_checking.html', {'error_sn': '输入不能为空'})
        if not Subbranch.objects.filter(name = subbranch_name):
            return render(request, 'bank_sys/add_checking.html', {'error_sn': '该支行不存在'})
        
        client_id = request.POST.get('client_id')
        if not client_id:
            return render(request, 'bank_sys/add_checking.html', {'error_ci': '输入不能为空'})
        if not Client.objects.filter(id = client_id):
            return render(request, 'bank_sys/add_checking.html', {'error_ci': '该客户不存在'})

        account_id = request.POST.get('account_id')
        if not account_id:
            return render(request, 'bank_sys/add_checking.html', {'error_ai': '输入不能为空'})
        if Account.objects.filter(id = account_id):
            return render(request, 'bank_sys/add_checking.html', {'error_ai': '该账户已存在'})
        ...
        if SubbranchClientAccountType.objects.filter(**{'subbranch_name': subbranch_name, 'client_id': client_id, 'account_type': 'checking_account'}):
            return render(request, 'bank_sys/add_checking.html', {'error_scat': '客户在该支行已拥有支票账户'})
    ...
    return render(request, 'bank_sys/add_checking.html')
...
def add_savings(request):
    if request.method == "POST":
        subbranch_name = request.POST.get('subbranch_name')
        if not subbranch_name:
            return render(request, 'bank_sys/add_savings.html', {'error_sn': '输入不能为空'})
        if not Subbranch.objects.filter(name = subbranch_name):
            return render(request, 'bank_sys/add_savings.html', {'error_sn': '该支行不存在'})
        
        client_id = request.POST.get('client_id')
        if not client_id:
            return render(request, 'bank_sys/add_savings.html', {'error_ci': '输入不能为空'})
        if not Client.objects.filter(id = client_id):
            return render(request, 'bank_sys/add_savings.html', {'error_ci': '该客户不存在'})

        account_id = request.POST.get('account_id')
        if not account_id:
            return render(request, 'bank_sys/add_savings.html', {'error_ai': '输入不能为空'})
        if Account.objects.filter(**{'id': account_id}):
            return render(request, 'bank_sys/add_savings.html', {'error_ai': '该账户已存在'})
        ...
        if SubbranchClientAccountType.objects.filter(**{'subbranch_name': subbranch_name, 'client_id': client_id, 'account_type': 'savings_account'}):
            return render(request, 'bank_sys/add_savings.html', {'error_scat': '客户在该支行已拥有储蓄账户'})
        ...
        return redirect('../accounts')
    return render(request, 'bank_sys/add_savings.html')
...
def del_account(request, account_id):
    if request.method == "GET":
        ...
        if obj_list[0]['type'] == 'checking_account' and obj_list[0]['balance'] < 0:
            return render(request, 'bank_sys/accounts.html', {'error_del': '透支额未付清，删除支票帐户失败', 'accounts': accounts, 'accessaccounts': accessaccounts})
...
def edit_account(request, account_id):
    ...
    if(request.method=="POST"):
        ...
        return redirect('../../accounts')
    return render(request, 'bank_sys/edit_account.html', {'obj':obj, 'subbranch':subbranch, 'client':client})
...
def add_clienttoaccount(request, account_id, account_type):
    ...
    if request.method == "POST":
        client_id = request.POST.get('client_id')
        if not Client.objects.filter(id=client_id):
            return render(request, 'bank_sys/add_clienttoaccount.html', {'error':'该客户不存在', 'subbranch':subbranch, 'account_id': account_id})
        if SubbranchClientAccountType.objects.filter(**{'subbranch_name': subbranch, 'client_id': client_id, 'account_type': account_type}):
            return render(request, 'bank_sys/add_clienttoaccount.html', {'error':'该客户已拥有该类型账户', 'subbranch':subbranch, 'account_id': account_id})
    ...
    return render(request, 'bank_sys/add_clienttoaccount.html', {'subbranch':subbranch, 'account_id': account_id})
...
def del_clienttoaccount(request, account_id, client_id):
    if request.method == "GET":
        ...
        if Client.objects.filter(id=client_id).count()==1:
            return render(request, 'bank_sys/accounts.html', {'error_del_c': '该账户仅有一个客户，不运行进行客户删除', 'accounts': accounts, 'accessaccounts': accessaccounts})
...
def loans(request):
    ...
    if request.method == "GET":
        ...
        return render(request, 'bank_sys/loans.html', {'loans':loans, 'payloans':payloans, "clientloans":clientloans})
    # 查询
    if request.method == "POST":
...
def add_loan(request):
    if request.method == "POST":
        loan_id = request.POST.get('loan_id')
        if not loan_id:
            return render(request, 'bank_sys/add_loan.html', {'error_li': '输入不能为空'})
        if Loan.objects.filter(id=loan_id):
            return render(request, 'bank_sys/add_loan.html', {'error_li': '该贷款号已存在'})

        subbranch_name = request.POST.get('subbranch_name')
        if not subbranch_name:
            return render(request, 'bank_sys/add_loan.html', {'error_sn': '输入不能为空'})
        if not Subbranch.objects.filter(name = subbranch_name):
            return render(request, 'bank_sys/add_loan.html', {'error_sn': '该支行不存在'})

        client_id = request.POST.get('client_id')
        if not client_id:
            return render(request, 'bank_sys/add_loan.html', {'error_ci': '输入不能为空'})
        if not Client.objects.filter(id = client_id):
            return render(request, 'bank_sys/add_loan.html', {'error_ci': '该客户不存在'})

        loan_amount = request.POST.get('loan_amount')
        if not loan_amount:
            return render(request, 'bank_sys/add_loan.html', {'error_la': '输入不能为空'})
    ...
    return render(request, 'bank_sys/add_loan.html')
...
def add_clientloan(request, loan_id):
    ...
    if request.method == "POST":
        client_id = request.POST.get('client_id')
        if not client_id:
            return render(request, 'bank_sys/add_clientloan.html', {'error': '输入不能为空'})
        if not Client.objects.filter(id=client_id):
            return render(request, 'bank_sys/add_clientloan.html', {'error_ci': '该客户不存在'})
        ...
        return redirect('../../loans')
    return render(request, 'bank_sys/add_clientloan.html', {'subbranch':subbranch, 'loan_id': loan_id})
...
def payloan(request, loan_id):
    ...
    if loan_status == 'issued':
        return render(request, 'bank_sys/loans.html', {'error_del': '该贷款已经全部发放', 'loans':loans, 'payloans':payloans, "clientloans":clientloans})
    if request.method == "POST":
        pay_date = request.POST.get('pay_date')
        pay_amount = request.POST.get('pay_amount')
        if Decimal(pay_amount) <= Decimal(0):
            return render(request, 'bank_sys/payloan.html', {'error_pa': '支付金额错误', 'loan_id': loan_id})
        ...
        if total_pay > Loan.objects.get(id=loan_id).amount:
            return render(request, 'bank_sys/payloan.html', {'error_pa': '支付金额超出贷款额度', 'loan_id': loan_id})
        ...
        return redirect('../../loans')
    return render(request, 'bank_sys/payloan.html', {'loan_id': loan_id})
...
def del_loan(request, loan_id):
    ...
    if request.method == "GET":
        ...
        if loan_status=='issuing':
            return render(request, 'bank_sys/loans.html', {'error_del': '该贷款正在支付中，无法删除', 'loans':loans, 'payloans':payloans, "clientloans":clientloans})
        loan = Loan.objects.filter(id=loan_id)
...
def statistics(request):
    ...
    return render(request, 'bank_sys/statistics.html', context)